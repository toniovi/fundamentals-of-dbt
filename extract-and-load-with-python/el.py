...
def download_data(active_datetime):
    ...
    file_path = f"../data-landing-zone/{url_datetime}.json.gz"
...
def load_data(incremental=False):
    if args.check:
        data_path = "../data-landing-zone/testing.json"
    else:
        data_path = "../data-landing-zone/*.json.gz"
...
parser.add_argument(
    ...
    default=datetime.strftime(datetime.utcnow() - timedelta(days=1, hours=1), "%Y-%m-%d-%H"),
    nargs="?",
    type=validate_date,
)
parser.add_argument(
